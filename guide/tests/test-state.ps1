...
Assert-True ($state -is [hashtable]) "New-GuideState returns hashtable"
...
# --- Add-GuideStatePhase ---
Add-GuideStatePhase -State $state -Name "Clone repos" -Type "loop"
Assert-Equal 1 $state.Phases.Count "Add-GuideStatePhase appends one phase"
...
Add-GuideStatePhase -State $state -Name "Check prereqs" -Type "single"
Assert-Equal 2 $state.Phases.Count "Second Add-GuideStatePhase brings count to 2"
Assert-Equal "single" $state.Phases[1].Type "Second phase is single"

# --- Set-GuideStatePhaseStatus ---
Set-GuideStatePhaseStatus -State $state -PhaseName "Clone repos" -Status "running"
Assert-Equal "running" $state.Phases[0].Status "Set-GuideStatePhaseStatus updates to running"

Set-GuideStatePhaseStatus -State $state -PhaseName "Clone repos" -Status "ok"
Assert-Equal "ok" $state.Phases[0].Status "Set-GuideStatePhaseStatus updates to ok"

Set-GuideStatePhaseStatus -State $state -PhaseName "Check prereqs" -Status "fail"
Assert-Equal "fail" $state.Phases[1].Status "Set-GuideStatePhaseStatus on second phase"

# --- Add-GuideStatePhaseItem ---
Add-GuideStatePhaseItem -State $state -PhaseName "Clone repos" -ItemName "api" -Status "ok" -Message "cloned"
Assert-Equal 1 $state.Phases[0].Items.Count "Add-GuideStatePhaseItem appends item"
...
Add-GuideStatePhaseItem -State $state -PhaseName "Clone repos" -ItemName "web" -Status "fail" -Message "timeout"
...
# --- Add-GuideStateIssue ---
Add-GuideStateIssue -State $state -Phase "Clone repos" -Severity "fail" -Message "web clone failed" -FixCommand "git clone web"
Assert-Equal 1 $state.Issues.Count "Add-GuideStateIssue appends issue"
...
Add-GuideStateIssue -State $state -Phase "Check prereqs" -Severity "warning" -Message "wrangler missing"
...
# --- Set-GuideStateActive ---
Set-GuideStateActive -State $state -Label "cloning web..."
Assert-Equal "cloning web..." $state.ActiveLabel "Set-GuideStateActive sets label"

Set-GuideStateActive -State $state -Label ""
Assert-Equal "" $state.ActiveLabel "Set-GuideStateActive can clear label"

# --- Get-GuideStateSummary ---
$summary = Get-GuideStateSummary -State $state
Assert-True ($summary -is [hashtable]) "Get-GuideStateSummary returns hashtable"
